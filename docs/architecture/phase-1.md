...
4. **Thực hiện bài thi:** giao bài theo sinh viên, kiểm tra thiết bị, thu audio/video theo từng câu, STT local ở Electron, STT server ở web, nộp transcript và tải minh chứng nền.
...
- **STT:** ưu tiên local trong Electron. Web dùng STT server để thử nghiệm không cần cài Python phía sinh viên. Đây là fallback đã dự kiến trong tài liệu gốc. Confidence Whisper là heuristic trung bình likelihood, chưa được hiệu chuẩn; transcript sửa tay có confidence 0 và cần xem lại.
...
Chưa triển khai local SQLite mã hóa, offline exam, khôi phục sau crash, upload queue bền vững, manual override/regrade, ghép video toàn bài, OCR, chống gian lận nâng cao, giám sát production và SSO. Những phần này thuộc Pilot/Production theo tài liệu gốc.
